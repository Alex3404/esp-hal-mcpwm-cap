...
mod mcpwm {
    use esp_hal::{
        ...
        mcpwm::{
            ...
            mcpwm0,
            timer::{CounterDirection, PwmWorkingMode, SyncOutSelect},
        },
        time::Rate,
    };

    struct Context<'d> {
        mcpwm: mcpwm0::McPwm<'static>,
        input: AnyPin<'d>,
        output: AnyPin<'d>,
        delay: Delay,
    ...
    fn test_capture_any_edge(mut ctx: Context<'static>) {
        // Setup capture 0 to to capture either falling or rising edges
        let mut output = Output::new(ctx.output, Level::Low, OutputConfig::default());
    ...
    #[test]
    fn test_timer_sync_phase_from_sync_line(mut ctx: Context<'static>) {
        ...
        // create timer but don't start it
        let mut timer = ctx.mcpwm.timer0;
        timer.set_sync_in(&ctx.mcpwm.sync0);
        timer.set_counter(0, CounterDirection::Increasing);
    ...
    #[test]
    fn test_cap_timer_sync_phase_from_sync_line(mut ctx: Context<'static>) {
        // setup sync line to be controlled by output pin
        let mut output = Output::new(ctx.output, Level::Low, OutputConfig::default());
        ctx.mcpwm.sync0.set_signal(ctx.input);

        let mut cap_timer = ctx.mcpwm.capture_timer;
        cap_timer.set_sync_in(&ctx.mcpwm.sync0);
    ...
    #[test]
    fn test_timer_sync_out_propagating_sync(mut ctx: Context<'static>) {
        // setup sync line to be controlled by output pin
        ctx.mcpwm.sync0.set_signal(ctx.input);
        ...
        // before sync
        assert_eq!(0, timer0.status().0);

        // timer 1 should be synced from timer 0
        timer1.set_sync_in(&timer0.sync_out);
